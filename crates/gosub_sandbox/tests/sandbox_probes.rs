...
mod sandbox_enforcement {
    ...
    /// The escape audit, per role: every attempt an attacker in that process
    /// would make comes out as the role's design says. Exit 3 = a violation;
    /// the report is on stderr.
    #[test]
    fn escape_audit_matches_each_roles_design() {
        for role in ["audit-renderer", "audit-net", "audit-service"] {
            let out = Command::new(probe_bin())
                .arg(role)
                .output()
                .expect("spawn sandbox-probe");
