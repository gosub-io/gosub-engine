...
fn run_platform_probe(probe: &str) {
    ...
    // The whole escape audit under one role's real lockdown: prints the
    // report, exits 3 on any violated expectation. The renderer and net rows
    ...
    if let Some(role) = probe.strip_prefix("audit-") {
        ...
        let (role, own) = match role {
            ...
            "service" => {
                let dir = std::env::temp_dir().join(format!("gosub-audit-service-{}", std::process::id()));
                ...
                (Role::Storage, vec![dir])
            }
            other => {
                eprintln!("unknown audit probe: {other}");
                std::process::exit(2);
            }
        };
        let report = run(role, &own);
