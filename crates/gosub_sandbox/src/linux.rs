...
fn install_with(
    ...
    // argument-filtered to "a thread, in no new namespace": `CLONE_NEW*` and
    // a plain fork hit the default action. Paired with `clone3` → ENOSYS by the caller,
    ...
    if threads_only && !fork_server {
        ...
        // ...and it must *be* a thread: a plain fork (neither flag) traps.
        let is_thread = SeccompCondition::new(
            0,
            SeccompCmpArgLen::Qword,
            SeccompCmpOp::MaskedEq(CLONE_VM | CLONE_THREAD),
            CLONE_VM | CLONE_THREAD,
        )?;
        rules.insert(
            libc::SYS_clone as i64,
            vec![SeccompRule::new(vec![no_namespace, is_thread])?],
        );
    }
