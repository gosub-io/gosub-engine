//! Named CSS colors and system color keywords, shared by the CSS system and the
//! render pipeline so both resolve names like `rebeccapurple` from the same table.
...
/// The hex value (`"#rrggbb"`) for a named CSS color, matched case-insensitively
/// as CSS keywords require. `None` when the name is not a CSS named color.
#[must_use]
pub fn named_color_hex(name: &str) -> Option<&'static str> {
    CSS_COLORNAMES
        .iter()
