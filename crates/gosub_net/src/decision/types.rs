...
pub enum HandlingDecision {
    /// Resource needs to be rendered based on its target.
    Render(RenderTarget),
    /// Resource should be downloaded to the given path.
    Download { path: PathBuf },
    /// Resource should be opened externally.
    OpenExternal,
...
pub enum BlockReason {
    /// The resource's MIME type is incompatible with the request destination.
    TypeMismatch,
    /// The response had `X-Content-Type-Options: nosniff` with an incompatible type.
    NosniffMismatch,
    /// The response MIME type was present but not recognized or supported.
    TypeUnknown,
