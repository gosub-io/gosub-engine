...
//! and scheduling HTTP requests.

use crate::decision_hub::{DecisionHub, DecisionToken};
...
use crate::types::{Action, RequestId};
use crate::utils::{short_url, Waiter};
use bytes::Bytes;
...
/// Manages a map of in-flight fetch requests.
...
/// Represents an item in the fetch queue.
struct QueueItem {
    req: FetchRequest,
    handle: FetchHandle,
...
pub struct Fetcher {
    ...
    /// Observer factory to create per-request observers
...
impl Fetcher {
    ...
    pub async fn run(&self, mut shutdown_tx: tokio::sync::watch::Receiver<bool>) {
        ...
        loop {
            ...
            // Setup the observer using the ObserverFactory
