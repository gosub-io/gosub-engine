use crate::engine::pipeline::font::LoadedFont;
use crate::engine::pipeline::js::LoadedScript;
...
#[derive(Debug)]
pub enum RoutedOutcome<C: HasDocument> {
    /// The main document has been parsed and is ready.
    MainDocument(Arc<C::Document>),
    ...
    CssLoaded(CssStylesheet),
    /// A script has been loaded (ready for execution by the JS engine).
    ScriptLoaded(LoadedScript),
    /// An image has been decoded.
    ImageDecoded(image::DynamicImage),
    /// A font has been loaded.
    FontLoaded(LoadedFont),
...
pub async fn route_response_for<C: HasDocument + Send + Sync + 'static>(
    ...
    match (dest, outcome.decision, body_content) {
        ...
        (RequestDestination::Script, HandlingDecision::Render(RenderTarget::JsEngine), body_content) => {
            ...
            Ok(RoutedOutcome::ScriptLoaded(script))
        }
