...
pub struct TabWorker<C: HasDocument> {
    /// ID of the tab
    pub tab_id: TabId,
    ...
    internal_tx: mpsc::Sender<TabInternalCommand<C>>,
    // Receive channel for internal commands
    internal_rx: mpsc::Receiver<TabInternalCommand<C>>,

    _phantom: PhantomData<C>,
}

impl<C: HasDocument + Send + Sync + 'static> TabWorker<C>
where
    C::Document: Send + Sync,
{
    ...
    pub fn new(
        ...
        let (internal_tx, internal_rx) = mpsc::channel::<TabInternalCommand<C>>(32);
    ...
    fn handle_internal_command(&mut self, cmd: TabInternalCommand<C>) {
        match cmd {
        ...
        tokio::spawn(async move {
            ...
            match outcome {
                Ok(RoutedOutcome::MainDocument(doc)) => {
                    // Extract final URL and title from the real document
                    let final_url = doc.url().unwrap_or_else(|| url.clone());
                    let title = extract_title::<C>(&doc);
