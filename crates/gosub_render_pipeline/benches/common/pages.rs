//! The fixture pages the style benchmark and the `style_dump` example both build on.
//!
//! A module rather than a bench of its own: it holds no measurement, only the HTML and CSS the
//! fixtures are made of. Both consumers pull it in with `#[path]`, so the page a dump compares
//! is byte-for-byte the page the benchmark measures.
...
pub fn generate_utility_page() -> GeneratedPage {
    GeneratedPage {
