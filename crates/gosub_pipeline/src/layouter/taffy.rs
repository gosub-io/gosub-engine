...
impl TaffyLayouter {
    ...
    fn process_inlines(
        ...
        for (inline_layout_element_id, inline_taffy_node_id) in current_inline_group {
            ...
            layout_tree
                .arena
                .get_mut(&anon_layout_id)
                .unwrap()
                .children
                .push(*inline_layout_element_id);
        }
    }
    ...
    fn generate_taffy_element(
        ...
        for child_id in render_node_children.iter() {
            ...
            self.process_inlines(&current_inline_group, &mut element_node, leaf_id, layout_tree);
    ...
    fn extract_taffy_data(&self, layout_tree: &LayoutTree, dom_node: &&Node) -> Option<(Option<TaffyContext>, Style)> {
        ...
        match &dom_node.node_type {
            ...
            NodeType::Text(text, node_style) => {
                ...
                if let Some(StyleValue::Unit(value, unit)) = node_style.get_property(StyleProperty::FontSize) { match unit {
                ...
                if let Some(StyleValue::Keyword(value)) = node_style.get_property(StyleProperty::FontFamily) { font_family = value.clone() }
