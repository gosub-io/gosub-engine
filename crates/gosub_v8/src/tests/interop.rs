...
impl TestStruct {
    ...
    fn variadic4<RT: WebRuntime>(_args: &RT::VariadicArgs) {}

    #[generic(I, i32, String)]
    fn variadic5<RT: WebRuntime, I: T1>(_i: I, _args: &RT::VariadicArgs, _ctx: &RT::Context) {}

    fn test222() {}

    fn uses_ctx(_ctx: &impl WebContext) {}
...
impl JSInterop for Test2 {
    ...
    fn implement<RT: WebRuntime>(s: Rc<RefCell<Self>>, mut ctx: RT::Context) -> Result<()> {
        let obj = RT::Object::new(&ctx)?;

        ctx.set_on_global_object("test2", obj.clone().into())?; //#name
        ...
        let cool_fn = {
            ...
            RT::Function::new(ctx.clone(), move |cb| {
                ...
                let ret = match s.borrow().cool_fn().to_web_value(ctx.clone()) {
                    Ok(ret) => ret,
                    Err(e) => {
                        cb.error(e);
                        return;
        ...
        let add = {
            ...
            RT::Function::new(ctx.clone(), move |cb| {
                ...
                let ret = s.borrow_mut().add(arg0 as i32).to_web_value(ctx.clone()).unwrap();
        ...
        let takes_ref = {
            ...
            RT::Function::new(ctx.clone(), move |cb| {
                ...
                let ret = s.borrow().takes_ref(&arg0).to_web_value(ctx.clone()).unwrap();
        ...
        let variadic = {
            ...
            RT::FunctionVariadic::new(ctx.clone(), move |cb| {
                ...
                #[allow(clippy::unit_arg)]
                let ret = s.borrow().variadic(&args).to_web_value(ctx).unwrap();
            ...
            RT::Function::new(ctx.clone(), move |cb| {
                ...
                if arg0.is_number() {
                    ...
                    if arg1.is_number() {
                        ...
                        let ret = s.borrow().generic(arg0, arg1).to_web_value(ctx.clone()).unwrap();
