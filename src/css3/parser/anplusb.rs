...
use crate::css3::tokenizer::{Number, TokenType};
use crate::css3::{Css3, Error};

impl Css3<'_> {

    fn do_dimension_block(&mut self, value: Number, unit: String) -> Result<(String, String), Error> {
    ...
    fn check_integer(&mut self, value: &str, offset: usize, allow_sign: bool) -> Result<bool, Error> {
    ...
    fn parse_anplusb_b(&mut self) -> Result<String, Error> {
        ...
        let negative = match self.tokenizer.lookahead(0).token_type {
            ...
            _ => {
                return Err(Error::new(
                    format!("Expected +, - or number, found {:?}", self.tokenizer.lookahead(0).token_type).to_string(),
                    self.tokenizer.current_location().clone(),
                ));
            }
        };
...
mod test {
    ...
    fn anplusb() {
        ...
        test!(parse_anplusb, "1n+2", Box::new(NodeType::AnPlusB { a: "1".to_string(), b: "2".to_string() } ));
        test!(parse_anplusb, "1n-2", Box::new(NodeType::AnPlusB { a: "1".to_string(), b: "-2".to_string() } ));
        test!(parse_anplusb, "-1n+2", Box::new(NodeType::AnPlusB { a: "-1".to_string(), b: "2".to_string() } ));
        test!(parse_anplusb, "-1n-20", Box::new(NodeType::AnPlusB { a: "-1".to_string(), b: "-20".to_string() } ));
        test!(parse_anplusb, "-1n+20", Box::new(NodeType::AnPlusB { a: "-1".to_string(), b: "20".to_string() } ));
        test!(parse_anplusb, "1n", Box::new(NodeType::AnPlusB { a: "1".to_string(), b: "0".to_string() } ));
        test!(parse_anplusb, "10n-5", Box::new(NodeType::AnPlusB { a: "10".to_string(), b: "-5".to_string() } ));
        test!(parse_anplusb, "0n+5", Box::new(NodeType::AnPlusB { a: "0".to_string(), b: "5".to_string() } ));
        test!(parse_anplusb, "1n+0", Box::new(NodeType::AnPlusB { a: "1".to_string(), b: "0".to_string() } ));
        test!(parse_anplusb, "n+0", Box::new(NodeType::AnPlusB { a: "1".to_string(), b: "0".to_string() } ));
        test!(parse_anplusb, "n", Box::new(NodeType::AnPlusB { a: "1".to_string(), b: "0".to_string() } ));
        test!(parse_anplusb, "2n+0", Box::new(NodeType::AnPlusB { a: "2".to_string(), b: "0".to_string() } ));
        test!(parse_anplusb, "2n", Box::new(NodeType::AnPlusB { a: "2".to_string(), b: "0".to_string() } ));
        test!(parse_anplusb, "3n-6", Box::new(NodeType::AnPlusB { a: "3".to_string(), b: "-6".to_string() } ));
        test!(parse_anplusb, "3n + 1", Box::new(NodeType::AnPlusB { a: "3".to_string(), b: "1".to_string() } ));
        test!(parse_anplusb, "+3n - 2", Box::new(NodeType::AnPlusB { a: "3".to_string(), b: "-2".to_string() } ));
        test!(parse_anplusb, "-n+ 6", Box::new(NodeType::AnPlusB { a: "-1".to_string(), b: "6".to_string() } ));
        test!(parse_anplusb, "-n+6", Box::new(NodeType::AnPlusB { a: "-1".to_string(), b: "6".to_string() } ));
        test!(parse_anplusb, "-n +6", Box::new(NodeType::AnPlusB { a: "-1".to_string(), b: "6".to_string() } ));
    }
}