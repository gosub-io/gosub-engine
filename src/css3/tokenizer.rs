...
/// The tokenizer is a component of the CSS parser that converts a stream of code points into a
/// stream of tokens. These tokens are then fed into the main CSS3 parser. It will however, not
/// genereate a final AST from the tokens, but rather groups these tokens into an intermediate
/// representation that is easier to parse for the different CSS modules.
///
/// Since the CSS parser has different entrypoints, it's possible that the parser is called with
/// an already existing stream of tokens instead of regular codepoints. To support this, the
/// tokenizer implements the `TokenStreamer` trait, which allows the parser to abstract the
/// tokenization process.
///
///   --- code points ----> tokenizer (implements TokenStreamer) --tokens -> parser
///   --- tokens ---------> tokenstream (implements TokenStreamer) --tokens -> parser
///
