...
-   **No GPU, no window system**: no wgpu adapter is requested and skia-safe's own C++ is statically linked, so the tool runs on a bare server or CI container with no display. It is not free of system libraries, though: Skia links fontconfig and freetype, so a Linux build needs `libfontconfig-dev` and `libfreetype-dev` and the runtime needs `libfontconfig.so.1` / `libfreetype.so.6`. (The winit-vello and egui-vello binaries genuinely need neither.)
