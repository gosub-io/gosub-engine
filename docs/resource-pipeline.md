...
The pipelines only see responses the router decided to *process*. `route_response_for` consults the request's destination and the zone's `UaPolicy` (MIME sniffing, PDF viewer, render-unknown-text-in-tab, download rules) first; responses that shouldn't be rendered go to the download path instead, surfacing as an `EngineEvent::DownloadRequested` offer --- see the navigation flow in [zones-and-tabs.md](zones-and-tabs.md). (Asking the UA up front via `DecisionRequired` is declared but not wired.) The networking stack underneath is the external **gosub-sonar** crate; the pipeline traits are the seam the engine keeps regardless of where the fetcher lives.
