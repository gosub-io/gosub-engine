...
-   **Downloads**: when a response arrives that isn't obviously a renderable page (content-type/disposition says download, unknown type, ...), the worker classifies it itself via `decide_handling`, cancels the navigation so the current document stays, and emits `EngineEvent::DownloadRequested` as an offer the UA can accept with `StartDownload` or ignore. (`NavigationEvent::DecisionRequired` --- the engine *asking* instead of deciding --- is declared but never emitted, and sits behind the `unstable-api` feature; only its answering half is wired.)
